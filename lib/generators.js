...
function _generateRoutes(controllers, custom_routes) {
    ...
    _.forEach(controllers, function (controller, identity) {
        ...
        controller = _.defaults(controller, {
            findOne: {http_method: 'get', path: identity + '/{id}'},
            find: {http_method: 'get', path: identity},
            create: {http_method: 'post', path: identity},
            update: {http_method: 'put', path: identity + '/{id}'},
            destroy: {http_method: 'delete', path: identity + '/{id}'}
        });
...
function _generatePaths(generated_routes, tags, customBlueprintMaps) {
    ...
    _.forEach(generated_routes, function (routes, identity) {
        _.forEach(routes, function (route) {
            ...
            var path = {
                summary: (route.action === "find" ? "List" : formatter.summary(route.http_method)) + " " + _.capitalize(identity),
            ...
            if (route.action === "create" || route.http_method === "update") {
