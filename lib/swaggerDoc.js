...
module.exports = function (sails, context) {
    ...
    var genPath = sails.config[context.configKey].swaggerJsonPath || sails.config.appPath + "/swagger/swagger.json";
    fs.writeFile(genPath, JSON.stringify(specifications), function (err) {
        if (err) {
